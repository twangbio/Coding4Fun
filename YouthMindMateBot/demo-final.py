...
def summary(llm, text):
    summary_prompt = (
        "Summarize the following text in ten to fifteen words. \n"
        f"Text: {text}\nSummary:"
    )
    messages = [
        (
            "system",
            "You are an expert for summarize the texts.",
        ),
...
negative_responses = [
    ...
    "I understand. Would you like to hear a joke or music? I can also suggest a short meditation.",
]
...
if prompt := st.chat_input("Say something"):
    ...
    with st.chat_message("assistant"):
        ...
        print(f"Raw Full response: {full_response}")

        if sentiment in ["negative"]:
            #full_reponse = summary(llm=llm, text=full_response)
            selector = random.randint(0, 10)
            print(f"Selector: {selector}")
            if selector >=0 and selector < 1 and jokes:
                joke = jokes.pop(0) 
                full_response = f"I'm sorry to hear that. Would you like to hear a joke or some relaxing music? \n Here's a joke for you: {joke['body']}"         
            
            elif selector >= 1 and selector < 2 and musics:
                song = musics.pop(0)        
                full_response = f"I understand. Would you like to hear a joke or music? I can also suggest a short meditation. \n \nHere is a music to relax {song['title']} with link: {song['link']}"
        
            elif selector >=2 and selector <= 7  and meditations:
                meditation = meditations.pop(0)
                full_response =  f"I understand. How about a short meditation? \n {meditation['instruction']}"
